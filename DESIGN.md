...
- [ ] 4シナリオ（カテゴリタブ・ページネーション・設定フロー・認証）が E2E テストでカバーされる
- [ ] 認証テストは `E2E_TEST_EMAIL` / `E2E_TEST_PASSWORD` が未設定の場合スキップされる
- [ ] `storageState` によるセッション再利用が機能する
- [ ] 既存の E2E テスト（9件）が引き続き通過する
