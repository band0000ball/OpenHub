...
- **データプレビュー（Phase 5.4）**: データセット詳細ページに JSON ツリー表示（折りたたみ対応）、
  CSV テーブル表示、プレーンテキスト表示を追加。データの中身を OpenHub 内で確認可能に。

- **気象庁コネクター（Phase 5.2）**: メタデータカタログ方式で天気予報（58 地域）・気象警報・地震情報を
  統合。静的 JSON カタログ内をキーワードマッチし、fetch 時に実 JMA エンドポイントを呼び出す。
  検索 API を持たないデータソースの統合パターンを確立。
