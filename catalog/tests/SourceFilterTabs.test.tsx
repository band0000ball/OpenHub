...
describe("SourceFilterTabs", () => {
  it("renders tablist with correct role", async () => {
    render(await SourceFilterTabs({ currentSource: "", currentQuery: "人口" }));
    expect(screen.getByRole("tablist")).toBeInTheDocument();
  });

  it("renders all three tabs: 全て, e-Stat, data.go.jp", async () => {
    render(await SourceFilterTabs({ currentSource: "", currentQuery: "人口" }));
  ...
  it("marks estat tab as selected when source=estat", async () => {
    render(await SourceFilterTabs({ currentSource: "estat", currentQuery: "人口" }));
    const eStatTab = screen.getByRole("tab", { name: "e-Stat" });
  ...
  it("marks datagojp tab as selected when source=datagojp", async () => {
    render(await SourceFilterTabs({ currentSource: "datagojp", currentQuery: "人口" }));
    const dataTab = screen.getByRole("tab", { name: "data.go.jp" });
    expect(dataTab).toHaveAttribute("aria-selected", "true");
  });

  it("e-Stat tab href contains source=estat and current query", async () => {
    render(await SourceFilterTabs({ currentSource: "", currentQuery: "人口" }));
    const eStatTab = screen.getByRole("tab", { name: "e-Stat" });
  ...
  it("全て tab href has no source param but includes query", async () => {
    render(await SourceFilterTabs({ currentSource: "estat", currentQuery: "人口" }));
    const allTab = screen.getByRole("tab", { name: "全て" });
  ...
  it("data.go.jp tab href contains source=datagojp", async () => {
    render(await SourceFilterTabs({ currentSource: "", currentQuery: "人口" }));
    const dataTab = screen.getByRole("tab", { name: "data.go.jp" });
